...
| ML engineering | Finite-value validation, device selection, seeded runs, state-aligned progress snapshots, input checks, and output collision protection |
...
- **Progress-reporting cost:** exact post-update snapshots require one additional no-gradient VGG forward pass at each reporting step. The default interval keeps that observability cost bounded; requesting very frequent callbacks adds runtime without changing the optimized pixels.
